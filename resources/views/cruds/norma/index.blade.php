            ...
            <ul class="list-group">
                @foreach($normas as $norma)
                    <li class="list-group-item align-items-center d-flex justify-content-between mt-2">ID #{{$norma->id}} |
                        Nome: {{$norma->nome}}
                        <span class="d-flex">
                            <form method="get" action="{{route("norma.show",["norma"=>$norma->id])}}">
                                @csrf
                                 <button class="btn btn-outline-primary mr-1"> <i class="fas fa-eye"></i></button>
                            </form>
                        <form method="get" action="{{route("norma.edit",["norma"=>$norma->id])}}">
                            @csrf
                            <button class="btn btn-outline-primary mr-1"> <i class="far fa-edit"></i> </button>
                        </form>
                        <form method="post" action="{{route("norma.remover",["id"=>$norma->id])}}" onsubmit="return confirm('Tem certeza que deseja excluir a norma {{$norma->nome}} ?')">
                            @csrf
                            @method('DELETE')
